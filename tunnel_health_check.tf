resource "null_resource" "tunnel_health_check" {
  ...
  provisioner "local-exec" {
    command = self.triggers.is_windows ? (
      "powershell.exe -ExecutionPolicy Bypass -File ${path.module}/scripts/check_tunnel_health.ps1 -TunnelID '${cloudflare_zero_trust_tunnel_cloudflared.tunnel[0].id}' -CloudflareAccountId '${var.cloudflare_account_id}' -CloudflareApiToken '${var.cloudflare_api_token}'"
      ) : (
      "${path.module}/scripts/check_tunnel_health.sh '${cloudflare_zero_trust_tunnel_cloudflared.tunnel[0].id}' '${var.cloudflare_account_id}' '${var.cloudflare_api_token}'"
    )
    interpreter = self.triggers.is_windows ? ["cmd", "/c"] : ["/bin/bash", "-c"]
  }
} 